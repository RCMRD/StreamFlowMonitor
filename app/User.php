...
class User extends Authenticatable
{
   ...
   /* public function role()
    ...
    static function dropdowns($user_id = NULL)
    {
        ...
        $data['user_roles_id_list']     = Role::orderBy('name', 'ASC')->pluck('name', 'id')->toArray();
	...
	static function consultants_dropdowns($user_id = NULL)
    {
        $select = __('form.dropdown_select_text');
	...
	static function dropdowns1($user_id = NULL)
    {
       
		
		$data['user_id_list'] = User::select(
                    DB::raw("CONCAT(first_name,' ',last_name) AS name"),'id')->pluck('name', 'id')->toArray();

        return $data;
    }
	
	static function dropdowns2()
    {
        $select = __('form.dropdown_select_text');

        $data['user_id_list'] = array('' => $select) + User::activeUsers()
                ->select(
                    DB::raw("CONCAT(first_name,' ',last_name) AS name"),'id')->pluck('name', 'id')->toArray();
