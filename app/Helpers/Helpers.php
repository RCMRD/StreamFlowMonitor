...
function a_links_project($project_id,$main_text, array $option_links)
{
    $data = $main_text."<div style='min-height: 25px;'><div class='row-options'>";

    foreach ($option_links as $link)
    {
        if(is_project_member($project_id))
        {
            $newTab = (isset($link['new_tab']) && $link['new_tab'] == TRUE) ? 'target="_blank"' : '';
            $data .= ' <a '.$newTab.' class="'.$link['action_class'].'" href="'.$link['action_link'].'">'.$link['action_text'].'</a>';    
        }
...
function a_links_team($team_id,$main_text, array $option_links)
{
    $data = $main_text."<div style='min-height: 25px;'><div class='row-options'>";

    foreach ($option_links as $link)
    {
        if(is_team_member($team_id))
        {
            $newTab = (isset($link['new_tab']) && $link['new_tab'] == TRUE) ? 'target="_blank"' : '';
            $data .= ' <a '.$newTab.' class="'.$link['action_class'].'" href="'.$link['action_link'].'">'.$link['action_text'].'</a>';    
        }
...
function a_links_proposol($proposol_id,$main_text, array $option_links)
{
    $data = $main_text."<div style='min-height: 25px;'><div class='row-options'>";

    foreach ($option_links as $link)
    {
        if(is_proposol_member($proposol_id))
        {
            $newTab = (isset($link['new_tab']) && $link['new_tab'] == TRUE) ? 'target="_blank"' : '';
            $data .= ' <a '.$newTab.' class="'.$link['action_class'].'" href="'.$link['action_link'].'">'.$link['action_text'].'</a>';    
        }
...
function a_links_eoi($eoi_id,$main_text, array $option_links)
{
    $data = $main_text."<div style='min-height: 25px;'><div class='row-options'>";

    foreach ($option_links as $link)
    {
        if(is_eoi_member($eoi_id))
        {
            $newTab = (isset($link['new_tab']) && $link['new_tab'] == TRUE) ? 'target="_blank"' : '';
            $data .= ' <a '.$newTab.' class="'.$link['action_class'].'" href="'.$link['action_link'].'">'.$link['action_text'].'</a>';    
        }
...
function a_links_task($task_id,$main_text, array $option_links)
{
    $data = $main_text."<div style='min-height: 25px;'><div class='row-options'>";

    foreach ($option_links as $link)
    {
        if(is_task_member($task_id))
        {
            $newTab = (isset($link['new_tab']) && $link['new_tab'] == TRUE) ? 'target="_blank"' : '';
            $data .= ' <a '.$newTab.' class="'.$link['action_class'].'" href="'.$link['action_link'].'">'.$link['action_text'].'</a>';    
        }
...
function delete_link_project($project_id,$delete_link)
{
	if(is_project_member($project_id))
        {
    return '<a class="delete_item btn btn-danger btn-sm" href="' . $delete_link . '"><i class="far fa-trash-alt"></i></a>';
...
function delete_link_proposol($proposol_id,$delete_link)
{
	if(is_proposol_member($proposol_id))
        {
    return '<a class="delete_item btn btn-danger btn-sm" href="' . $delete_link . '"><i class="far fa-trash-alt"></i></a>';
...
function delete_link_eoi($eoi_id,$delete_link)
{
	if(is_eoi_member($eoi_id))
        {
    return '<a class="delete_item btn btn-danger btn-sm" href="' . $delete_link . '"><i class="far fa-trash-alt"></i></a>';
        ...
        <div class="row bottom-toolbar">
            <div  class="col-md-12">
                <div style="text-align: right;">
                    <input type="submit" class="btn btn-primary" onclick="waitingDialog.show('Please Wait', {dialogSize: 'sm', progressType: 'success'})" value="<?php echo $text; ?>"/>
...
define("DEFAULT_USER_PASSWORD", 1234567);
...
define("COMPONENT_TYPE_PROPOSOL", 16);
define("COMPONENT_TYPE_EOI", 17);
// Task Statuses
...
// Proposol Statuses
define("AWAITING_APPROVAL_FOR_DEVELOPMENT", 1);
define("DECLINED_FOR_DEVELOPMENT", 2);
define("APPROVED_FOR_DEVELOPMENT", 3);
define("AWAITING_APPROVAL_FOR_SUBMISSION", 4);
define("APPROVED_FOR_SUBMISSION", 5);
define("NOT_AWARDED", 6);
define("AWARDED", 7);


// Eoi Statuses
define("EOI_AWAITING_APPROVAL_FOR_DEVELOPMENT", 1);
define("EOI_DECLINED_FOR_DEVELOPMENT", 2);
define("EOI_APPROVED_FOR_DEVELOPMENT", 3);
define("EOI_AWAITING_APPROVAL_FOR_SUBMISSION", 4);
define("EOI_APPROVED_FOR_SUBMISSION", 5);
define("EOI_NOT_SELECTED", 6);
define("EOI_SELECTED", 7);
...
define("PROJECT_STATUS_COMPLETED", 5);


// Project Statuses Chemutt
define("PROPOSAL_DEVELOPMENT", 1);
define("PROPOSAL_SUBMITTED", 2);
//define("NOT_AWARDED", 3);
define("PROJECT_IMPLEMENTATION", 4);
define("PROJECT_COMPLETED", 5);
...
define("LOG_NAME_PROPOSOL", 'proposal_');

define("LOG_NAME_EOI", 'eoi_');
...
function activity_log_name_by_componet_id($component_id)
{
    $data = [
		...
		COMPONENT_TYPE_PROPOSOL     => 'proposal_',
		COMPONENT_TYPE_EOI     => 'eoi_',
...
function get_url_route_name_by_component_id($component_id)
{
    $data = [
		...
		COMPONENT_TYPE_PROPOSOL       => 'show_proposol_page',
		COMPONENT_TYPE_EOI       => 'show_eoi_page',
    ];
...
function is_project_member($project_id)
{
	...
	/*$check=FALSE;
	$pro=\App\ProjectMember::where('project_id',$project_id)->where('user_id',auth()->user()->id)->first();
	if(isset($pro)){
	 $check=TRUE;	
	}else{
		$proc=\App\Project::where('created_by',auth()->user()->id)->first();
		if(isset($proc)){
			$check=TRUE;
		}else{
	...
	$count = $project->members()->where('user_id', auth()->user()->id )->get()->count();
	//return $count = $project->members()->get();

            if(!(($project->created_by == auth()->user()->id ) ||  $count > 0 || auth()->user()->hasRole('Administrator')))
            {          
                return FALSE;
            }else{
			return TRUE;	
...
function is_proposol_member($proposol_id)
{
	...
	$proposol=\App\Proposol::find($proposol_id);
	
	$count = $proposol->members()->where('user_id', auth()->user()->id )->get()->count();
	//return $count = $project->members()->get();

            if(!(($proposol->created_by == auth()->user()->id ) ||  $count > 0 || auth()->user()->hasRole('Administrator')))
            {          
                return FALSE;
            }else{
			return TRUE;	
...
function is_team_member($team_id)
{
	
	$team=\App\Team::find($team_id);
	
	$count = $team->members()->where('user_id', auth()->user()->id )->get()->count();
	//return $count = $project->members()->get();

            if(!($count > 0 || auth()->user()->hasRole('Administrator')))
            {          
                return FALSE;
            }else{
			return TRUE;	
...
function is_eoi_member($eoi_id)
{
	

	$eoi=\App\Eoi::find($eoi_id);
	
	$count = $eoi->members()->where('user_id', auth()->user()->id )->get()->count();
	//return $count = $project->members()->get();

            if(!(($eoi->created_by == auth()->user()->id ) ||  $count > 0 || auth()->user()->hasRole('Administrator')))
            {          
                return FALSE;
            }else{
			return TRUE;	
...
function is_task_member($task_id)
{
	...
	$count = $related->members()->where('user_id', auth()->user()->id )->get()->count();
	//return $count = $project->members()->get();

            if(!(($related->created_by == auth()->user()->id )||($task->created_by == auth()->user()->id )||($task->assigned_to == auth()->user()->id ) ||  $count > 0 || auth()->user()->hasRole('Administrator')))
            {          
                return FALSE;
            }else{
			return TRUE;	
